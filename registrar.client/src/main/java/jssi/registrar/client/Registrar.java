...
public class Registrar implements UniRegistrar {
    ...
    public static final URI DEFAULT_REGISTER_URI = URI.create("http://localhost:8080/registrar/1.0/register");
    public static final URI DEFAULT_UPDATE_URI = URI.create("http://localhost:8080/registrar/1.0/update");
    public static final URI DEFAULT_DEACTIVATE_URI = URI.create("http://localhost:8080/registrar/1.0/deactivate");
    public static final URI DEFAULT_PROPERTIES_URI = URI.create("http://localhost:8080/1.0/registrar/properties");
    ...
    public RegisterState register(String driverId, RegisterRequest registerRequest) throws RegistrationException {
        ...
        String uriString = getRegisterUri().toString() + "?driverId=" + urlEncode(driverId);
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("Request for register request " + registerRequest + " to: " + uriString);
        }
        ...
        try ( CloseableHttpResponse httpResponse = (CloseableHttpResponse) this.getHttpClient().execute(httpPost)) {
            ...
            if (httpResponse.getStatusLine().getStatusCode() > 200) {
                if (LOG.isWarnEnabled()) {
                    LOG.warn("Cannot retrieve REGISTER STATE for register request " + registerRequest + " from " + uriString + ": " + httpBody);
                }
        ...
        LOG.debug("Retrieved REGISTER STATE for for register request " + registerRequest + " (" + uriString + "): " + registerState);
    ...
    public UpdateState update(String driverId, UpdateRequest updateRequest) throws RegistrationException {
        ...
        // prepare HTTP request
        String uriString = getUpdateUri().toString() + "?driverId=" + urlEncode(driverId);
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("Request for update request " + updateRequest + " to: " + uriString);
        }
        ...
        try ( CloseableHttpResponse httpResponse = (CloseableHttpResponse) this.getHttpClient().execute(httpPost)) {
            ...
            if (httpResponse.getStatusLine().getStatusCode() > 200) {
                ...
                if (LOG.isWarnEnabled()) {
                    LOG.warn("Cannot retrieve UPDATE STATE for update request " + updateRequest + " from " + uriString + ": " + httpBody);
                }
                throw new RegistrationException(httpBody);
            }
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("Retrieved UPDATE STATE for update request " + updateRequest + " (" + uriString + "): " + updateState);
        }
    ...
    public DeactivateState deactivate(String driverId, DeactivateRequest deactivateRequest) throws RegistrationException {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("Request for deactivate request " + deactivateRequest + " to: " + uriString);
        }
        ...
        try ( CloseableHttpResponse httpResponse = (CloseableHttpResponse) this.getHttpClient().execute(httpPost)) {
            ...
            if (httpResponse.getStatusLine().getStatusCode() > 200) {
                ...
                if (LOG.isWarnEnabled()) {
                    LOG.warn("Cannot retrieve DEACTIVATE STATE for deactivate request " + deactivateRequest + " from " + uriString + ": " + httpBody);
                }
                throw new RegistrationException(httpBody);
            }
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("Retrieved DEACTIVATE STATE for for deactivate request " + deactivateRequest + " (" + uriString + "): " + deactivateState);
        }
